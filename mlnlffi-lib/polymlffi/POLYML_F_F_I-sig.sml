...
signature POLYML_F_F_I =
  sig
    ...
    val symbolFromAddress: (unit -> Memory.Pointer.t) -> symbol

    (* Dynamic loading functions
     *
     * `getSymbol` is equivalent to `getSymbolFromLibrary (loadExecutable ())`
     * and is provided for convenience.  `getSymbolFromLibrary lib name` is
    ...
    val getSymbolFromLibrary : library -> string -> symbol
    ...
    val cSize            : word conversion  (* `word` to match `Memory.malloc` *)
    ...
    val closureWithAbi :
      Foreign.LowLevel.abi -> ('as, 'r) func -> ('as -> 'r) -> ('as -> 'r) closure
    val closure : ('as, 'r) func -> ('as -> 'r) -> ('as -> 'r) closure
    val nullClosure : ('as -> 'r) closure

    val callWithAbi : Foreign.LowLevel.abi -> ('as, 'r) func -> symbol -> 'as -> 'r
    val call : ('as, 'r) func -> symbol -> 'as -> 'r
  end
